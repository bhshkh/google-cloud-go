...
func saveSliceProperty(props *[]Property, name string, opts saveOpts, v reflect.Value) error {
	...
	// Work out the properties generated by the first non-empty element in the slice. This will
	// usually be a single property, but will be more if this is a slice of structs.
	var headProps []Property
	var headIndex int
	for headIndex = 0; headIndex < v.Len(); headIndex++ {
		if err := saveStructProperty(&headProps, name, opts, v.Index(headIndex)); err != nil {
			return err
		}
		if len(headProps) > 0 {
			break
	...
	// If all elements were empty, we're done.
	if len(headProps) == 0 {
		return nil
	}
