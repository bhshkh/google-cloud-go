...
// RunQuery runs the given query in the given context with the provided options
func (c *Client) RunQuery(ctx context.Context, query QueryType, opts ...RunOption) (it *Iterator) {
	ctx = trace.StartSpan(ctx, "cloud.google.com/go/datastore.Query.RunQuery")
	defer func() { trace.EndSpan(ctx, it.err) }()
	return c.run(ctx, query, opts...)
}

// run runs the given query in the given context with the provided options
func (c *Client) run(ctx context.Context, query QueryType, opts ...RunOption) *Iterator {
	var q *Query
...
type FindNearestOptions struct {
	...
	// If the property already exists in the entity, its value get overwritten with the distance calculation.
