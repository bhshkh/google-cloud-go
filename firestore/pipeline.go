...
func (fdo *funcDeleteOption) applyStage(do map[string]any) {
	fdo.f(do)
}

func (*funcDeleteOption) isDeleteOption() {}

func newFuncDeleteOption(f func(map[string]any)) *funcDeleteOption {
	return &funcDeleteOption{
		f: f,
	}
}
...
// Experimental: Firestore Pipelines is currently in preview and is subject to potential breaking changes in future versions,
// regardless of any other documented package stability guarantees.
func WithUpdateTransformations(fields []Selectable) UpdateOption {
	return newFuncUpdateOption(func(ao map[string]any) {
		ao["transformations"] = fields
	})
}
...
func (p *Pipeline) Update(opts ...UpdateOption) *Pipeline {
	...
	if t, ok := options["transformations"].([]Selectable); ok {
		transformations = t
		delete(options, "transformations")
	}

	stage, err := newUpdateStage(transformations, options)
	if err != nil {
		p.err = err
		return p
	}
